  ...
  describe "#redirect_url_for_subschema" do
    ...
    context "when the subschema has a group" do
      ...
      it "should generate a url with the subschema's step" do
        assert_equal redirect_url_for_subschema(subschema, content_block_edition), content_block_manager_content_block_workflow_path(content_block_edition, step: "#{Workflow::Step::SUBSCHEMA_PREFIX}#{subschema.id}")
      end
    end
    ...
    context "when the subschema has no group" do
      ...
      it "should generate a url with the subschema's group" do
        assert_equal redirect_url_for_subschema(subschema, content_block_edition), content_block_manager_content_block_workflow_path(content_block_edition, step: "#{Workflow::Step::GROUP_PREFIX}#{subschema.group}")
      end
    end
  end
end
