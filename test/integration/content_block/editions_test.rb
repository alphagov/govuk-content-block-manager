...
class ContentBlockManager::ContentBlock::EditionsTest < ActionDispatch::IntegrationTest
  ...
  describe "#new" do
    ...
    describe "when a document id is provided" do
      ...
      it "initializes the form for the latest edition" do
        ...
        visit new_content_block_manager_content_block_document_edition_path(content_block_document)

        assert_text content_block_document.title
      end
    end

    describe "when a document id is not provided" do
      it "initializes the form for the latest edition" do
        edition = create(:content_block_edition, :pension)
        ...
        visit new_content_block_manager_content_block_edition_path(block_type: "block-type")
  ...
  describe "#create" do
    ...
    let(:content_block_document) { create(:content_block_document, :pension) }
    let!(:original_edition) { create(:content_block_edition, :pension, document: content_block_document, title:) }
    ...
    describe "when updating an existing block" do
      it "creates a new content block with params generated by the schema" do
        assert_changes -> { content_block_document.editions.count }, from: 1, to: 2 do
          assert_changes -> { ContentBlockManager::ContentBlock::Version.count }, from: 1, to: 2 do
            post content_block_manager_content_block_document_editions_path(content_block_document), params: {
              something: "else",
      ...
      it "should render the template when a validation error is raised" do
        renders_errors do
          ...
          assert_equal assigns(:title), content_block_document.title
        end
      end
    ...
    describe "when creating a new block" do
      ...
      it "creates a new content block with params generated by the schema" do
        post content_block_manager_content_block_editions_path, params: {
          something: "else",
          "content_block/edition": {
            document_attributes:,
            details:,
...
def renders_errors(&block)
  content_block_edition = build(:content_block_edition, :pension, document: content_block_document)
...
def redirects_to_step(step, &block)
  content_block_edition = build(:content_block_edition, :pension, id: 123)
